...
function i_am_asus() {
  [ $(uname) = "Linux" ] && [ $(hostname) == "asus-g14" ] && [ $(whoami) = "mayday" ]
}
...
colour_my_prompt () {
    ...
    # Check if __git is non-empty and non-whitespace
    if [[ -n "${__git_ps1// /}" ]]; then
        export PS1="$__icon $__chroot_debian $__host $__current_dir $__git $__line_end"
    else
        export PS1="$__icon $__chroot_debian $__host $__current_dir $__line_end"
    fi
